...
def CoordConv2D(data, num_filter, kernel, stride, pad, no_bias, name):
    ...
    out = mx.sym.Convolution(data=new_input, num_filter=num_filter, kernel=kernel, stride=stride, pad=pad,no_bias=no_bias, name=name)

    return out

def get_conv(coord=True):
    if coord:
        return CoordConv2D
    else:
